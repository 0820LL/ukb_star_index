...
def steward(config_file_path:str, ukb_star_index_path:str, send_message_script:str) -> None:
    ...
    ukb_star_index_command = 'nextflow run -offline -profile singularity -bg -params-file {} {} >> run_log.txt'.format(params_file_path, ukb_star_index_path)
    return_value = os.system(ukb_star_index_command)
    with open('ukb_star_index_command.txt', 'w') as command_f:
        command_f.write(ukb_star_index_command + '\n')
    ...
    while True:
        with open(execution_trace_file, encoding = 'UTF-8') as trace_f:
            for line in trace_f:
                if 'STAR_GENOMEGENERATE' in line:
                    if 'COMPLETED' in line:
                        feedback_dict['endDate'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                        feedback_dict['error'] = 0
                        for file in os.listdir('{}/results'.format(analysis_path)):
                            if file.endswith('star_index.tar.gz'):
                                star_index = '{}/results/{}'.format(analysis_path, file)
                        report_download_dict = {
                            ...
                            'text': [
                                {
                                    'sort':1,
                                    'title': 'STAR构建的index文件',
                                    'content': 'STAR构建的index文件：#&{}'.format(star_index), 
                                    'postDes': '', 
                                    'memo': '', 
                                    'preDes': ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_star_index_path', required=True, help='the full path of the ukb_star_index')
    ...
    ukb_star_index_path = args.ukb_star_index_path
    send_message_script = args.send_message_script
    steward(config_file_path, ukb_star_index_path, send_message_script)
